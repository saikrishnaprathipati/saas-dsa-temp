...
public class EmailValidator {
	...
	public EmailValidator(AdvisorLookupService advisorLookupService) {
		this.advisorLookupService = advisorLookupService;
	}

	public void validate(Object target, Errors errors) {
		logger.info("ForgotPasswordEmailValidator {}", target);

		ForgotPasswordFormVO forgotPasswordFormVO = (ForgotPasswordFormVO) target;
		validateEmailAddress(errors, forgotPasswordFormVO);
	}

	private void validateEmailAddress(Errors errors, ForgotPasswordFormVO forgotPasswordFormVO) {
		String email = forgotPasswordFormVO.getEmailAddress().toLowerCase();
		if (StringUtils.isBlank(email)) {
			errors.rejectValue(EMAIL_ADDRESS, DSA_EMAIL_ADDRESS_REQUIRED);
		} else if (!matches(EMAIL_REGEX, forgotPasswordFormVO.getEmailAddress())) {
